...
try 
{
    ...
    $query = "SELECT f.matricula,f.identificacion,f.nombres,f.apellidos,f.email,       
        ...
        left join ( select c.username,c.email,
                    ...
                    from ec_diagnostico_teorico_practico c
        ...
        left join (Select c.username,
                   ...
                   from ec_satisfaccion_teorico_practico c 
                   where c.respuesta not in ('NULL')
                   and c.tipoEstudiante = 'ESPOL'
                   group by c.username) i
        on  f.email like concat('%',i.username,'%')

        left join rubricasexcelteoricopractico h 
        ...
        left join rubricaspptteoricopractico j 
        ...
        left join rubricaswordteoricopractico k 
        ...
        while($fila = mysqli_fetch_assoc($result))
        {
            ...
            if ($puntosExtras > 0)
            {
                ...
                if ($cohorte == 'PRACTICO')
                {
                    ...
                    if ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienPPT < $puntosLlegarCienWord && $puntosLlegarCienPPT <= 5 )
                    {
                        $PPT  = $calificacionPPT + $puntosLlegarCienPPT;
                        $Word = $calificacionWord + (5 - $puntosLlegarCienPPT);
                    }

                    if ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienWord < $puntosLlegarCienPPT && $puntosLlegarCienWord <= 5 )
                    {
                        $Word  = $calificacionWord + $puntosLlegarCienWord;
                        $PPT = $calificacionPPT + (5 - $puntosLlegarCienWord);
                    }

                    if ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienPPT < $puntosLlegarCienWord && $puntosLlegarCienPPT > 5)
                    {
                        $PPT  = $calificacionPPT + $sumaMas5;
                        $Word = $calificacionWord;
                    }
                    ...
                    if (($calificacionWord > 60 && $calificacionPPT   < 60 && $puntosPasarPPT   > 10 && $calificacionExcel < 60 && $puntosPasarExcel > 10) ||
                        ...
                        ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienWord < $puntosLlegarCienPPT && $puntosLlegarCienWord <= 5)||
                        ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienPPT < $puntosLlegarCienWord && $puntosLlegarCienPPT <= 5) ||
                        ($calificacionExcel == 100 && $calificacionPPT > 59 && $calificacionWord > 59 && $puntosLlegarCienPPT < $puntosLlegarCienWord && $puntosLlegarCienPPT > 5))
            ...
            if ($eliminaData) {
                eliminarDatosPuntosExtrasTeoricoPracticos($termino,$anio);
                $eliminaData = false;
            }

            if ($procesar > 0) 
            {
                guardar_datos_puntos_extras_teorico_practico($matricula,$identificacion,$nombres,$apellidos,$email,$calificacionExcel,$calificacionPPT,$calificacionWord,$demografica,$diagnostico,$satisfaccion,$puntosExtras,$Excel,$PPT,$Word,$termino,$anio,$sistema,$tipoEstudiante,$cohorte,$franja,$paralelo);
