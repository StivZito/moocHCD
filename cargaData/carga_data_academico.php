...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
	try 
	{
			...
			{	
				...
				foreach ( $xlsx->rows() as $k => $r ) 
				{
					...
					for ( $i = 0; $i < $cols; $i ++ ) 
					{
						...
						guardarDatosAcademico($matricula,$identificacion,$nombres,$apellidos,$email,$sexo,$fecha_nac,$edad,$estado_civil,$colegio,$tipo_colegio,$notaTeorico,$notaWord,$notaExcel,$notaPpt,$numVeces,$carrera,$termino,$anio,$sistema,$tipo,$tipoEstudiante,$archivo);					
				} 
			} else {
		...
		echo 'Archivo Procesado Exitosamente '.$termino.'-'.$anio.'-'.$sistema.'-'.$tipo.'-'.$tipoEstudiante.'-'.$archivo;
