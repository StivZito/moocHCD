			...
			{	
				...
				foreach ( $xlsx->rows() as $k => $r ) 
				{
					...
					for ( $i = 0; $i < $cols; $i ++ ) 
					{
						...
						if (isset($r[20]) && $r[20] !=='') $p18 = utf8_encode($r[20]); else $p18 =  0; 
						if (isset($r[21]) && $r[21] !=='') $p19 = utf8_encode($r[21]); else $p19 =  0; 
						if (isset($r[22]) && $r[22] !=='') $p20 = utf8_encode($r[22]); else $p20 =  0; 
						if (isset($r[23]) && $r[23] !=='') $p21 = utf8_encode($r[23]); else $p21 =  0; 
						if (isset($r[24]) && $r[24] !=='') $p22 = utf8_encode($r[24]); else $p22 =  0; 
						if (isset($r[25]) && $r[25] !=='') $p23 = utf8_encode($r[25]); else $p23 =  0; 

						if (isset($r[26]) && $r[26] !=='') $calificacion = utf8_encode($r[26]); else $calificacion =  0; 						
					...
					if ($cedula !== ''){
						guardarDatosRubricaPPTTeoricoPractico($id,$estudiante,$cedula,$p1,$p2,$p3,$p4,$p5,$p6,$p7,$p8,$p9,$p10,$p11,$p12,$p13,$p14,$p15,$p16,$p17,$p18,$p19,$p20,$p21,$p22,$p23,$calificacion,$termino,$anio,$sistema,$adminEspol,$franja,$paralelo,$docente);			
					}
				} 
			} else {
